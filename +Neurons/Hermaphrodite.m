classdef Hermaphrodite
    ...
    methods (Static)
        ...
        function is_neuron = isNeuron(name)
            ...
            is_neuron = ...
                ~isempty(find(strcmp(name, Neurons.Hermaphrodite.getNeurons()),1));
        end
        ...
        function is_cell = isCell(name)
            ...
            % Is this AWC?
            %is_cell = Neurons.Hermaphrodite.isAWC(name);
            is_cell = false;
            ...
            if ~is_cell
                is_cell = ...
                    ~isempty(find(strcmp(name, Neurons.Hermaphrodite.non_neuronal_cells),1));
            end
        end
        ...
        function [name, LR] = stripLR(name)
            ...
            if (name(end) ~= 'L' && name(end) ~= 'R') || ...
                    any(strcmp(name, Neurons.Hermaphrodite.non_LR_neurons))
                return;
            end
        ...
        function names = neuronStartsWith(str)
            ...
            names = [];
            neurons = Neurons.Hermaphrodite.getNeurons();
        ...
        function color = getNeuronColor(name)
            ...
            color = [];
            [names, colors] = Neurons.Hermaphrodite.getColors();
        ...
        function name = getNeuroPALColorClass(name)
            ...
            if ~Neurons.Hermaphrodite.isNeuron(name)
                name = [];
                return;
            end
            
            % Is the neuron deterministically asymetrically colored?
            if any(strcmp(name, Neurons.Hermaphrodite.asym_deterministic_LR_neurons))
                return;
            end
            
            % Strip the neuron of its L/R designation.
            name = Neurons.Hermaphrodite.stripLR(name);
            
            % Does the neuron have 2, 4, or 6 fold symmetry?
            if name(end) == 'D' || name(end) == 'V'
                nameNoDV = name(1:(end-1));
                if any(strcmp(nameNoDV, Neurons.Hermaphrodite.sym_2_fold_DV_neurons)) || ...
                        any(strcmp(nameNoDV, Neurons.Hermaphrodite.sym_4_fold_DV_neurons)) || ...
                        any(strcmp(nameNoDV, Neurons.Hermaphrodite.sym_6_fold_DV_neurons))
                    name = [nameNoDV 'DV'];
                end
            end
        end
        ...
        function neuron_class = getNeuronClass(neuron)
            ...
            % Find the neuron.
            neuron_class = [];
            neuron_i = strcmpi(neuron, Neurons.Hermaphrodite.getNeurons());
            if all(neuron_i == 0)
                return;
            end
            ...
            classes = Neurons.Hermaphrodite.getClasses();
        ...
        function neuron = getNeuroPALName(neuron)
            ...
            % Is this a neuron?
            if ~Neurons.Hermaphrodite.isNeuron(neuron)
                neuron = [];
                return;
            end
        ...
        function num_neurons = numNeurons(body)
            ...
            switch lower(strtrim(body))
                case 'whole worm'
                    num_neurons = 300;
                case 'head'
                    num_neurons = Neurons.Hermaphrodite.numHeadNeurons();
                case 'midbody'
                    num_neurons = Neurons.Hermaphrodite.numMidbodyNeurons();
                case 'anterior midbody'
                    num_neurons = Neurons.Hermaphrodite.numAnteriorMidbodyNeurons();
                case 'central midbody'
                    num_neurons = Neurons.Hermaphrodite.numCentralMidbodyNeurons();
                case 'posterior midbody'
                    num_neurons = Neurons.Hermaphrodite.numPosteriorMidbodyNeurons();
                case 'tail'
                    num_neurons = Neurons.Hermaphrodite.numTailNeurons();
            end
        end
        ...
        function num_neurons = numHeadNeurons()
            ...
            persistent num;
            if isempty(num)
                num = length(Hermaphrodite.getAnteriorPharynx()) + ...
                    length(Hermaphrodite.getPosteriorPharynx()) + ...
                    length(Hermaphrodite.getLeftAnteriorGanglion()) + ...
                    length(Hermaphrodite.getRightAnteriorGanglion()) + ...
                    length(Hermaphrodite.getDorsalGanglion()) + ...
                    length(Hermaphrodite.getLeftLateralGanglion()) + ...
                    length(Hermaphrodite.getRightLateralGanglion()) + ...
                    length(Hermaphrodite.getVentralGanglion()) + ...
                    length(Hermaphrodite.getRetroVesicularGanglion());
            end
            num_neurons = num;
        end
        ...
        function num_neurons = numMidbodyNeurons()
            ...
            persistent num;
            if isempty(num)
                num = length(Hermaphrodite.getAnteriorMidbody()) + ...
                    length(Hermaphrodite.getCentralMidbody()) + ...
                    length(Hermaphrodite.getPosteriorMidbody()) + ...
                    length(Hermaphrodite.getVentralNerveCord());
            end
            num_neurons = num;
        end
        ...
        function num_neurons = numAnteriorMidbodyNeurons()
            ...
            if isempty(num)
                num = length(Hermaphrodite.getAnteriorMidbody()) + ...
                    round(length(Hermaphrodite.getVentralNerveCord()) / 3);
            end
            num_neurons = num;
        end
        ...
        function num_neurons = numCentralMidbodyNeurons()
            ...
            persistent num;
            if isempty(num)
                num = length(Hermaphrodite.getCentralMidbody()) + ...
                    round(length(Hermaphrodite.getVentralNerveCord()) / 3);
            end
            num_neurons = num;
        end
        ...
        function num_neurons = numPosteriorMidbodyNeurons()
            ...
            persistent num;
            if isempty(num)
                num = length(Hermaphrodite.getPosteriorMidbody()) + ...
                    round(length(Hermaphrodite.getVentralNerveCord()) / 3);
            end
            num_neurons = num;
        end
        ...
        function num_neurons = numTailNeurons()
            ...
            persistent num;
            if isempty(num)
                num = length(Hermaphrodite.getPreAnalGanglion()) + ...
                    length(Hermaphrodite.getDorsoRectalGanglion()) + ...
                    length(Hermaphrodite.getLeftLumbarGanglion()) + ...
                    length(Hermaphrodite.getRightLumbarGanglion());
            end
            num_neurons = num;
        end
        ...
        function names = getGanglionNames()
            ...
            if isempty(ganglion_names)
                ganglia = Neurons.Hermaphrodite.getGanglia();
        ...
        function names = getAnteriorPharynx()
            ...
            if isempty(neurons)
                ganglia = Neurons.Hermaphrodite.getGanglia();
                i = find(strcmp(Neurons.Hermaphrodite.getGanglionNames(), ...
