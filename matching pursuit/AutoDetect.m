classdef AutoDetect < Singleton
   ...
   methods(Static)
      ...
      function [c, ceq] = constrain_eigenvalues(x, norm, init, bound)
        ...
        function resid = fit_gaussian_cov(x, vol, norm, mu, color_props, baseline_props)
        ...
        % Amin Nejat
            ...
            recon = simulate_gaussian(sz(1: 3), ...
                mu', variances, props', baseline', zeros(1, size(vol, 4)), obj.trunc);

            resid = sqrt(sum((vol(:) - recon(:)).^2));
        end
        
        function [shape, recon, mu, cov, colors] = get_gaussian_cov(x, norm, mu, color_props)
        % Creating Gaussian function for a given set of parameters, used for
        % visualization.
        %
        % Amin Nejat
        
            sz = [2*obj.fsize+1, obj.szext(4)];
            ...
            recon = simulate_gaussian(sz, ...
                mu, cov, colors, zeros(1, sz(4)), zeros(1, sz(4)), obj.trunc);
